...
@Component({
  selector: 'app-election-report-ins', 
  templateUrl: './election-report-ins.component.html',
  styleUrls: ['./election-report-ins.component.scss']
})
export class ElectionReportInsComponent {
  ...
  getElectionRequestList() {
    ...
    this.service.getElectionRequestListForInspReportList(params).subscribe({
      next: (response: any) => {
        debugger
  ...
  {
    this.service.GetSelectedExecutiveMembers({electionId: this.report.electionRequestId,createdBy:this.id}).subscribe({
      next:(response:any)=>{
