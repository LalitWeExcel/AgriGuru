...
function rstr_hmac_md5(key: string, data: string): string {
  ...
  let hash: number[] = binl_md5(ipad.concat(rstr2binl(data)), 512 + data.length * 8);
...
function rstr2b64(input: string): string {
  ...
  for (let i = 0; i < len; i += 3) {
    let triplet: number = (input.charCodeAt(i) << 16) | (i + 1 < len ? input.charCodeAt(i + 1) << 8 : 0) | (i + 2 < len ? input.charCodeAt(i + 2) : 0);
    for (let j = 0; j < 4; j++) {
...
function rstr2any(input: string, encoding: string): string {
   ...
   * use.
   */
  let full_length: number = Math.ceil((input.length * 8) / (Math.log(encoding.length) / Math.log(2)));
