...
function AddEditAds(Idx) {
    location.href = $_AddEditAdsUrl + "?Id=" + Idx;
}
...
function GetIndependent(Id) {
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#ImageThumbnail1').attr('src', result.ImageName);
            $('#ImageThumbnail2').attr('src', result.VideoName);

            $('#txttitle_Independent').val(result.Title);
            $('#txtMRP_Independent').val(result.MRP);
            $('#txtDescription_Independent').val(result.Description);
            $('#txtDiscount_Independent').val(result.Discount);
            $('#txtDiscountInRupees_Independent').val(result.DiscountInRupees);

            var Discount = $('#txtDiscount_Independent').val();
            var DiscountInRupees = $('#txtDiscountInRupees_Independent').val();

            if (Discount != 0) {
                $('.percentage').removeClass('hide')
                $('.amount').addClass('hide')
                $('#ddDiscountType option:eq(0)').prop('selected', true);
            }
            else if (DiscountInRupees != 0) {
                $('.percentage').addClass('hide')
                $('.amount').removeClass('hide')
                $('#ddDiscountType option:eq(1)').prop('selected', true);
            }
...
function SaveUpdateIndependentads() {
    var isValid = true;
    var fileData = new FormData();
    var Ids = $("#hddId").val();

    var title = $("#txttitle_Independent").val().trim();
    var mrp = $("#txtMRP_Independent").val().trim();
    var Discount = $("#txtDiscount_Independent").val().trim();
    var DiscountInRupees = parseInt($("#txtDiscountInRupees_Independent").val().trim());
    var Description = $("#txtDescription_Independent").val().trim();
    var DiscountType = $("#ddDiscountType option:selected").val();


    if (title == "") {
        $('#spntitle_Independent').html('Please Enter Title !');
        $('#spntitle_Independent').removeClass('hide');
        isValid = false;
    }
    else if (mrp == "") {
        $('#spnMRP_Independent').html('Please Enter MRP !');
        $('#spnMRP_Independent').removeClass('hide');
        $('#spntitle_Independent').addClass('hide');
        isValid = false;
    }

    else if (mrp < 1 || mrp > 10000) {
        $('#spnMRP_Independent').html('Discount should be max Rs. 10000 /.');
        $('#spnMRP_Independent').removeClass('hide');
        $('#spntitle_Independent').html('');
        isValid = false;
    }

    else if (DiscountType === "percentage" && Discount < 1 || Discount > 100) {
        $('#spnDiscount_Independent').html('Discount should be max 100 %.');
        $('#spnDiscount_Independent').removeClass('hide');
        $('#spnMRP_Independent').html('');
        $('#spntitle_Independent').html('');
        DiscountInRupees = 0;
        isValid = false;
    }
    else if (DiscountType === "amount" && DiscountInRupees > mrp) {
        $('#spnDiscountInRupees_Independent').html('Discount In Rupees should be less then MRP amount ' + mrp);
        $('#spnDiscountInRupees_Independent').removeClass('hide');
        $('#spntitle_Independent').html('');
        $('#spnMRP_Independent').html('');
        Discount = 0;
        isValid = false;
        ...
        $('#spnDescription_Independent').html('Description should be max 100 digit.');
        $('#spnDescription_Independent').removeClass('hide');
        $('#spnDiscount_Independent').html('');
    ...
    if (window.FormData !== undefined) {
        var fileUpload = $("#upload-image1").get(0);
        var files = fileUpload.files;
        ...
        if (files.length > 0) {
            ...
            if (Ids == 0) {
                $('#spnImageName1').html('Please Select  Image Thumbnail');
                $('#spnImageName1').removeClass('hide');
                $('#spnDiscountInRupees_Independent').html('');
                isValid = false;
            }
        }
    ...
    if (!isValid) {
        ...
        $.ajax({
            url: $_SaveUpdateIndependentadsUrl,
            data: fileData,
            type: "POST",
            contentType: false, // Not to set any content header
            processData: false, // Not to process data,
...
function Delete(id) {
    ConfirmationMsg('Are you sure, you want to delete this Advertising?', function () {
...
function OpenPopup(Ids, type) {
    ...
    $("#hddId").val(Ids);
    $("#title").html('Edit')
    $("#Independentadstitle").html('Edit Independent ads')

    if (type === "Independent Ads") {
        GetIndependent(Ids);
    } else {
        AddEditAds(Ids)
       // Get(Ids)
...
function ClearFields() {
    ...
    $("#title").html('Add')
    $("#Independentadstitle").html('Add Independent ads')
    $("#hddId").val('');
    $('#spnDiscount').html('');
    $('#spnActivate').html('');
    $('#spnSubCatagory').html('');
    ...
    $("#txttitle_Independent").val('');
    $("#spntitle_Independent").html('');
    $("#txtMRP_Independent").val('');
    $("#spnMRP_Independent").html('');
    $("#txtDiscount_Independent").val('');
    $("#spnDiscount_Independent").html('');
    $("#txtDiscountInRupees_Independent").val('');
    $("#spnDiscountInRupees_Independent").html('');
    $("#txtDescription_Independent").val('');
    $("#spnDescription_Independent").html('');


    $('#AddCatagory option:eq(0)').prop('selected', true);
    $('#SubCatagory option:eq(0)').prop('selected', true);

    $('#AddState option:eq(0)').prop('selected', true);
    $('#ddState option:eq(0)').prop('selected', true);

    $('#ddDistrict option:eq(0)').prop('selected', true);
    $('#AddTimeSlot option:eq(0)').prop('selected', true);

    $('#ImageThumbnail').attr('src', $_Image);
    $('#ImageThumbnail1').attr('src', $_Image);
...
$(function () {
    ...
    $("#ddDiscountType").change(function () {
        var value = $(this).val();
        $('#txtDiscount_Independent').val('');
        $('#txtDiscountInRupees').val('');
...
$('#upload-image1').on('change', function (e) {
    ...
    var ext1 = '.' + ext.toLowerCase();
    if (img_ext.includes(ext1)) {
        var selected_file = $('#upload-image1').get(0).files[0];
        selected_file = window.URL.createObjectURL(selected_file);
        $('#ImageThumbnail1').attr('src', selected_file);
        // return true;
    } else {
...
function ActivateDeactivate(isactive) {
    ...
    $('input:checkbox:checked', tableControl).each(function () {
        ...
        if (newsId !== null && newsId > 0) {
            ids.push({ Id: newsId.toString() });
        }
    });

    $.ajax({
        url: $_ActivateDeactivate,
        data: JSON.stringify({ 'advertising_ids': ids, 'IsActive': isactive }),
        type: 'POST',
        datatype: 'json',
        contentType: 'application/json; charset=utf-8',
        success: function (data) {
...
function Activate(isactive) {
    ConfirmationMsg('Are you sure, do you want to activate this advertising ?', function () {
        ActivateDeactivate(isactive);
    });
}

function Deactivate(dactivate) {
    ConfirmationMsg('Are you sure, do you want to deactivate this advertising ?', function () {
